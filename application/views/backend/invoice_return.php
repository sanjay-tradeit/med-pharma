...
.table th, .table thead th{padding-left:15px;}
      ...
      <div class="col-12">
        <div class="card card-outline-info" style="border-radius: none;">
                ...
                <div class="col-md-12">
                 <form action="Sales_Return_Form" method="post" class="SalesFormConfirm" id="SalesFormConfirm" enctype="multipart/form-data">
                  <div class="pos_inputs">
                      <div class="row m-b-5">
                        ...
                        <div class="col-md-3 p-l-5">
                           <div class="input-group" >
                            ...
                            <input type="text" class="form-control sdate" name="sdate" id="sdate" placeholder="Sales Date" readonly value="<?php echo $salesreport->create_date; ?>">
                          </div>
                        </div>
                        <div class="col-md-3">
                          <div class="row">
                      ...
                      <div class="row">
                        <div class="col-md-12">
                          <div class="table-responsive mb-15" style="height: 500px; overflow-y: auto; ">
                            <table class="table table-bordered m-t-5 purchase">
                                        ...
                                        <?php foreach($allSales as $value): 
                                          ...
                                          $GethsnNum = $CI->GethsnNum($value->mid);
                                          $hsn = ($GethsnNum[0]->hsn);
                                          $getigst = $CI->getigst($hsn);
                                          $igst = ($getigst[0]->igst);
                                          $tasx = (($value->qty)* $igst)/100;
                                           //print_r($value);
                                        $discount = ($value->rate * $value->gdiscount)/100;
                                        $discount2 = ($value->rate * $value->discount)/100;
                                        
                                        $rate =   $value->rate - $discount - $discount2; 
                                          ?>
                                        ...
                                        <tr>
                                            ...
                                            <input type="hidden" class="form-control" name="supplier_id[]"  value="<?php echo $value->supplier_id; ?>">
                                            <td><input type="text" class="form-control" name="genname[]" placeholder="Ounce" readonly="" value="<?php echo $value->generic_name ?>"></td>
                                            <td><input type="text" class="form-control" name="batch[]"  value="<?php echo $value->Batch_Number; ?>"></td>
                                            ...
                                            <td><input type="text" class="form-control sale" name="sale[]" placeholder="" value="<?php echo $rate;//echo round($value->total_price / $value->qty,2) ?>" readonly=""></td>
                                            <td><input type="hidden" class="form-control igst" name="igst" id="igst"   value="<?php echo $igst; ?>"><input type="text" class="form-control deduction" name="deduction[]" placeholder="" value="0"></td>
                                            <td><input type="hidden" class="form-control tasx" name="tax[]" id="tax"   value="0"><input type="text" class="form-control total" name="total[]" placeholder="" value="0" readonly></td>
          ...
          $(document).on('keyup','.rqty, .total, .gtotal, .deduction,.granddeduction',function() { 
            ...
            var returnQ = parseInt($(reqty).val());
            var returnPrice = parseInt($(price).val()); 
           var igst =  $('#igst').val();

           
           var tax = ((returnPrice * returnQ)*igst)/100;
          // alert(tax);
           ...
           // var tax = $('#tax').val();
            //alert(tax);
                    ...
                    //alert(sumtax);
                 var total_sum =    parseFloat(sumtax) + parseFloat(sum);
           ...
           //  var treturn =  parseFloat(treturn) +  parseFloat(total_sum);
          //  alert(total_sum);
          //  alert(treturn);
          var tax =  parseFloat(treturn);
              $(".returntotal").val(tax);

              var totalTax = parseFloat($(".tax").val());
            var tax = isNaN(totalTax) ? 0 : totalTax * rqtyval;
            // Update tax field
            $(".tax").val(tax);
          });
        });
