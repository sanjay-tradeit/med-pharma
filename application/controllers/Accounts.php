...
class Accounts extends CI_Controller {
        ...
        if($this->session->userdata('user_login_access') != False) {
            ...
            $this->form_validation->set_error_delimiters();
                ...
                $success = $this->user_model->UPDATE_ACCOUNT($id,$data);                                   
                    ...
                    $response['status'] = 'success';
                    $response['message'] = "Successfully Payment Receipt";
                    $response['curl'] = base_url()."Accounts/all_payment";
                    $this->output->set_output(json_encode($response));                          
                    } 
                   // redirect('https://browncrystal.com/mad-pharma/accounts/all_payment');                 
                }
    ...
    public function all_payment()
    {
        ...
        $data['payment'] = $this->supplier_model->all_payment(); 
        // echo "<pre>";
        // print_r($data['payment']);
       $this->load->view('backend/all_payment', $data);
    } else {
        redirect(base_url().'Sales/auth_error', 'refresh');
    }
    }
